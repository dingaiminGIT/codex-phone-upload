...
enum MobileUploadPage {
    static func html(expiresAt: Date, targetName: String) -> String {
        ...
        const copy={
          en:{title:'Send images to Codex',targetLabel:'Target task',noSelection:'Select at least one image.',unsupported:'Only PNG, JPEG, GIF, WebP, HEIC, and HEIF images are supported.',selected:n=>`${n} image${n===1?'':'s'} selected`,choose:'Choose images',more:'Choose more',empty:'Choose screenshots or photos from your phone.',ready:'Ready',notice:'Images are placed in the composer and are not sent automatically.',submit:'Upload to Codex',uploading:p=>`Uploading… ${p}%`,attaching:'Upload complete. Attaching images to Codex…',success:n=>`${n} image${n===1?' was':'s were'} attached to the Codex composer.`,partial:(a,r)=>`${a} attached. Continue with the remaining ${r}.`,failure:'Upload failed. Try again.',tooMany:`Choose up to ${MAX_FILES} images.`,tooLarge:'Each image must be 25 MB or smaller.',totalTooLarge:'The selected images must total 100 MB or less.',limits:'Up to 12 images · 25 MB each · 100 MB total',expires:'Link expires at',remove:'Remove'},
          'zh-Hans':{title:'传图片到 Codex',targetLabel:'目标任务',noSelection:'请至少选择一张图片。',unsupported:'只支持 PNG、JPEG、GIF、WebP、HEIC 和 HEIF 图片。',selected:n=>`已选择 ${n} 张`,choose:'选择图片',more:'继续选择',empty:'从手机中选择截图或照片。',ready:'已就绪',notice:'图片只会放入输入框，不会自动发送。',submit:'上传到 Codex',uploading:p=>`正在上传… ${p}%`,attaching:'上传完成，正在放入 Codex…',success:n=>`已将 ${n} 张图片放入 Codex 输入框。`,partial:(a,r)=>`已放入 ${a} 张，请继续上传剩余 ${r} 张。`,failure:'上传失败，请重试。',tooMany:`一次最多选择 ${MAX_FILES} 张图片。`,tooLarge:'单张图片不能超过 25 MB。',totalTooLarge:'所选图片总计不能超过 100 MB。',limits:'最多 12 张 · 每张 25 MB · 总计 100 MB',expires:'链接失效时间',remove:'移除'}
        };
