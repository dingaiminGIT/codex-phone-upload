...
enum MobileUploadPage {
    static func html(expiresAt: Date, targetName: String, mode: UploadMode) -> String {
        let formatter = ISO8601DateFormatter()
        let expires = formatter.string(from: expiresAt)
        let escapedTarget = escapeHTML(targetName)
        let englishNetworkNotice = mode == .local
            ? "Local transfer uses unencrypted HTTP. Use only on a trusted Wi-Fi network."
            : "Public transfer uses a temporary Cloudflare HTTPS link. Keep this page private."
        let chineseNetworkNotice = mode == .local
            ? "局域网传输使用未加密的 HTTP，请仅在可信 Wi-Fi 下使用。"
            : "公网传输使用临时 Cloudflare HTTPS 链接，请勿分享此页面。"
        return """
        <!doctype html>
        <html lang="en">
        <head>
        ...
        <main>
          ...
          <form id="form" method="post" enctype="multipart/form-data">
            ...
            <div class="network-notice" data-i18n="networkNotice">\(englishNetworkNotice)</div>
